...
export default class Parameters extends React.Component {
  static propTypes = {
    ...
    queryBasedParameters: PropTypes.any,
  };

  static defaultProps = {
    parameters: [],
    queryBasedParameters: [],
    editable: false,
    sortable: false,
  ...
  setPendingValue = (param, value, isDirty) => {
    if (value) {
      this.props.globalParameters[param.name] = value;

      for(let i=0;i<this.props.queryBasedParameters.length;i++) {
        if (this.props.queryBasedParameters[i].props.parameter.name !== param.name) {
          this.props.queryBasedParameters[i].componentDidMount();
        }
      }
    }
  ...
  registerParameterItem = (item, unmount) => {
    for(let i=0;i<this.props.queryBasedParameters.length;i++) {
      if (this.props.queryBasedParameters[i].props.parameter.name === item.props.parameter.name) {
        if (unmount) {
          this.props.queryBasedParameters.splice(i, 1);
        }
    ...
    this.props.queryBasedParameters.push(item);
  };
    ...
    return (
      <div key={param.name} className="di-block" data-test={`ParameterName-${param.name}`}>
        ...
        <ParameterValueInput
          ...
          registerParameterItem={(item, unmount) => this.registerParameterItem(item, unmount)}
        />
      </div>
    );
  }
